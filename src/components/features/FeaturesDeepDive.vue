  ...
  <section class="py-24 px-6">
    <div class="max-w-7xl mx-auto space-y-24">
      ...
      <div class="grid lg:grid-cols-2 gap-14 items-center">
        <div>
          <p class="text-xs font-bold text-brand-500 uppercase tracking-widest mb-4">{{ $t('featuresDeepDive.analyticsTagline') }}</p>
          <h2 class="font-display text-4xl sm:text-5xl font-extrabold mb-5">
            {{ $t('featuresDeepDive.analyticsH2a') }}<br/><span class="text-gradient">{{ $t('featuresDeepDive.analyticsH2b') }}</span>
          </h2>
          <p class="text-lg text-zinc-500 dark:text-zinc-400 mb-8 leading-relaxed">
            {{ $t('featuresDeepDive.analyticsSub') }}
          </p>
          ...
          <a href="/register?plan=premium" class="btn-primary inline-flex items-center gap-2">
            {{ $t('featuresDeepDive.analyticsCtaBtn') }}
        ...
        <div class="relative">
          ...
          <div class="relative card shadow-xl overflow-hidden">
            ...
            <div class="grid grid-cols-3 divide-x divide-zinc-100 dark:divide-zinc-800 border-b border-zinc-100 dark:border-zinc-800">
              <div class="p-4 text-center"><div class="font-display text-2xl font-bold">24</div><div class="text-xs text-zinc-400 mt-0.5">{{ $t('featuresDeepDive.dashDownloads') }}</div></div>
              <div class="p-4 text-center"><div class="font-display text-2xl font-bold">17</div><div class="text-xs text-zinc-400 mt-0.5">{{ $t('featuresDeepDive.dashUniqueVisitors') }}</div></div>
              <div class="p-4 text-center"><div class="font-display text-2xl font-bold text-red-500">3</div><div class="text-xs text-zinc-400 mt-0.5">{{ $t('featuresDeepDive.dashFlaggedIPs') }}</div></div>
            </div>
            ...
            <div class="relative px-6 py-4 border-t border-zinc-100 dark:border-zinc-800">
              ...
              <div class="absolute inset-0 flex items-center justify-center">
                <a href="/register?plan=pro" class="btn-primary text-xs">{{ $t('featuresDeepDive.unlockBtn') }}</a>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="grid lg:grid-cols-2 gap-14 items-center">
          ...
          <div class="relative card shadow-xl p-8 space-y-5">
            <div v-for="sec in securityMock" :key="sec.label"
              ...
              <div class="flex items-center gap-1.5 text-xs font-bold text-emerald-600 dark:text-emerald-400">
                ...
                {{ $t('featuresDeepDive.activeLabel') }}
              </div>
            </div>
          </div>
        </div>
        ...
        <div class="order-1 lg:order-2">
          <p class="text-xs font-bold text-violet-500 uppercase tracking-widest mb-4">{{ $t('featuresDeepDive.securityTagline') }}</p>
          <h2 class="font-display text-4xl sm:text-5xl font-extrabold mb-5">
            {{ $t('featuresDeepDive.securityH2a') }}<br/><span class="text-gradient">{{ $t('featuresDeepDive.securityH2b') }}</span>
          </h2>
          <p class="text-lg text-zinc-500 dark:text-zinc-400 mb-8 leading-relaxed">
            {{ $t('featuresDeepDive.securitySub') }}
          </p>
          ...
          <a href="/register" class="btn-primary inline-flex items-center gap-2">
            {{ $t('featuresDeepDive.secureCtaBtn') }}
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 8l4 4m0 0l-4 4m4-4H3"/>
            </svg>
          </a>
...
const analyticsBenefits = computed(() => [
  { title: t('featuresDeepDive.benefit0_title'), text: t('featuresDeepDive.benefit0_text'), icon: 'M15 12a3 3 0 11-6 0 3 3 0 016 0z M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z', bg: 'bg-brand-50 dark:bg-brand-900/20', color: 'text-brand-500' },
  { title: t('featuresDeepDive.benefit1_title'), text: t('featuresDeepDive.benefit1_text'), icon: 'M18.364 18.364A9 9 0 005.636 5.636m12.728 12.728L5.636 5.636', bg: 'bg-red-50 dark:bg-red-900/20', color: 'text-red-500' },
  { title: t('featuresDeepDive.benefit2_title'), text: t('featuresDeepDive.benefit2_text'), icon: 'M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z', bg: 'bg-violet-50 dark:bg-violet-900/20', color: 'text-violet-500' },
])
const mockVisitors = [
...
const securityMock = computed(() => [
  { label: t('featuresDeepDive.secMock0_label'), desc: t('featuresDeepDive.secMock0_desc'), icon: 'M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z', bg: 'bg-blue-50 dark:bg-blue-900/20',    color: 'text-blue-500' },
  { label: t('featuresDeepDive.secMock1_label'), desc: t('featuresDeepDive.secMock1_desc'), icon: 'M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z', bg: 'bg-violet-50 dark:bg-violet-900/20', color: 'text-violet-500' },
  { label: t('featuresDeepDive.secMock2_label'), desc: t('featuresDeepDive.secMock2_desc'), icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z', bg: 'bg-amber-50 dark:bg-amber-900/20',  color: 'text-amber-500' },
  { label: t('featuresDeepDive.secMock3_label'), desc: t('featuresDeepDive.secMock3_desc'), icon: 'M13 10V3L4 14h7v7l9-11h-7z', bg: 'bg-red-50 dark:bg-red-900/20',      color: 'text-red-500' },
  { label: t('featuresDeepDive.secMock4_label'), desc: t('featuresDeepDive.secMock4_desc'), icon: 'M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z', bg: 'bg-emerald-50 dark:bg-emerald-900/20', color: 'text-emerald-500' },
])
const securityBenefits = computed(() => [
  t('featuresDeepDive.secBenefit0'),
  t('featuresDeepDive.secBenefit1'),
  t('featuresDeepDive.secBenefit2'),
  t('featuresDeepDive.secBenefit3'),
  t('featuresDeepDive.secBenefit4'),
])
</script>