<template>
  <main class="min-h-screen bg-white dark:bg-zinc-950">
    ...
    <section class="border-b border-zinc-200 dark:border-zinc-800 py-16 px-6 bg-zinc-50 dark:bg-zinc-900/50">
      <div class="max-w-4xl mx-auto">
        ...
        <h1 class="font-display text-4xl sm:text-5xl font-extrabold text-zinc-900 dark:text-zinc-50 mb-4">API Documentation</h1>
    ...
    <div class="max-w-4xl mx-auto px-6 py-14 space-y-16">
      ...
      <section>
        ...
        <div v-if="!isAuth" class="rounded-2xl border border-zinc-200 dark:border-zinc-800 p-8 text-center">
          <p class="text-zinc-500 dark:text-zinc-400 mb-4">Sign in to manage your API key.</p>
          <a href="/login" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-brand-500 text-white font-bold text-sm hover:bg-brand-400 transition-colors">Sign In</a>
        </div>
        ...
        <div v-else-if="!isPro" class="rounded-2xl border border-amber-200 dark:border-amber-800/40 bg-amber-50 dark:bg-amber-900/10 p-8">
          <div class="flex items-start gap-4">
            ...
            <div>
              <p class="font-semibold text-amber-900 dark:text-amber-300 mb-1">Pro or Pro Max plan required</p>
              <p class="text-sm text-amber-700 dark:text-amber-400 mb-4">API access is available on Pro ($9.99/mo) and Pro Max ($19.99/mo) plans.</p>
              <a href="/pricing" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl bg-amber-500 text-white font-bold text-sm hover:bg-amber-400 transition-colors">Upgrade Plan →</a>
            </div>
          </div>
        </div>
        ...
        <div v-else class="space-y-4">
          ...
          <div class="rounded-2xl border border-zinc-200 dark:border-zinc-800 p-6">
            <div class="flex items-center justify-between mb-3">
              <span class="text-sm font-semibold text-zinc-700 dark:text-zinc-300">API Key</span>
            ...
            <p v-else class="text-sm text-zinc-500 dark:text-zinc-400">No API key generated yet.</p>
            ...
            <p v-if="copied" class="text-xs text-green-600 dark:text-green-400 mt-3">✓ Copied to clipboard</p>
            <p v-if="error" class="text-xs text-red-500 mt-3">{{ error }}</p>
          </div>

          <p class="text-xs text-zinc-400">
            Keep your API key secret. Regenerating creates a new key and immediately invalidates the old one.
          </p>
        </div>
      </section>
      ...
      <!-- Authentication -->
      <section>
        <h2 class="font-display text-2xl font-bold text-zinc-900 dark:text-zinc-50 mb-2">Authentication</h2>
  ...
  -H <span class="text-green-400">"Authorization: Bearer fstr_YOUR_API_KEY"</span></pre>
      ...
      <!-- Base URL -->
      <section>
        <h2 class="font-display text-2xl font-bold text-zinc-900 dark:text-zinc-50 mb-2">Base URL</h2>
      ...
      <!-- Endpoints -->
      <section>
        <h2 class="font-display text-2xl font-bold text-zinc-900 dark:text-zinc-50 mb-8">Endpoints</h2>
      ...
      <!-- Rate limits -->
      <section>
        <h2 class="font-display text-2xl font-bold text-zinc-900 dark:text-zinc-50 mb-4">Rate Limits</h2>
      ...
      <!-- Error codes -->
      <section class="pb-12">
        <h2 class="font-display text-2xl font-bold text-zinc-900 dark:text-zinc-50 mb-4">Error Codes</h2>
        <div class="rounded-2xl border border-zinc-200 dark:border-zinc-800 divide-y divide-zinc-200 dark:divide-zinc-800">
